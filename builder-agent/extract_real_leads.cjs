...
async function extractRealLeads() {
  ...
  // 1. Find Top 4 Industries
  const distribution = {};
  leads.forEach(l => {
    let ind = l.industry || 'Unknown';
    if (ind === 'tham_my_vien') ind = 'spa'; // merge spa and tham_my_vien
  ...
  const top4 = sortedIndustries.slice(0, 4);
  console.log("Top 4 real industries found:");
  top4.forEach(([ind, count]) => console.log(`- ${ind}: ${count}`));
  
  const targetIndustries = top4.map(i => i[0]);
  
  // 2. Select exactly 50 from each of the Top 4
  const selectedLeads = [];
  ...
  for (const ind of targetIndustries) {
    ...
    // Sort or just slice the first 50
    selectedLeads.push(...industryLeads.slice(0, 50));
  }
  
  console.log(`Selected ${selectedLeads.length} real leads.`);
  
  // 3. Clear old dummy tenants (but keep Admin or very old ones if needed, actually let's just wipe all generated demos that don't match our real leads)
  console.log("Wiping old dummy tenants...");
  await supabase.from('tenants').delete().neq('slug', 'tavi'); // delete all except 'tavi' (main site)
  
  // 4. Create new tenants for these real leads
  const newTenants = selectedLeads.map(lead => {
    return {
      id: lead.id, // match ID!
      name: lead.name,
      slug: stringToSlug(lead.name),
      template_key: lead.industry === 'tham_my_vien' ? 'spa' : lead.industry, // map to valid template key
  ...
  if (insertErr) {
    ...
    console.log("Successfully created real tenants for the 200 leads!");
  }
  
  // 5. Export to Excel & Markdown
  const formattedData = selectedLeads.map((l, index) => {
    ...
    return {
      ...
      'Link Demo': `https://taviweb.vercel.app/${stringToSlug(l.name)}`
    };
  });
  ...
  console.log("Done exporting real leads.");
}
