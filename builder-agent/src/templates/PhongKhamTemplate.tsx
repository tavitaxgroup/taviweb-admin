...
export default function PhongKhamTemplate({ lead }: { lead: any }) {
  return (
    <div>
      {/* Hero Section */}
      <section className="relative pt-20 pb-32 overflow-hidden bg-slate-50">
        <div className="absolute inset-0 z-0 opacity-10 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-emerald-600 via-slate-50 to-slate-50"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-lg text-slate-600 mb-8 leading-relaxed max-w-xl">
              Chăm sóc sức khỏe toàn diện cho bạn và gia đình. Chúng tôi tự hào mang đến trải nghiệm tuyệt vời nhất cho bạn.
            </p>
            <div className="flex flex-wrap gap-4">
              <button className="bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-4 rounded-full font-bold shadow-lg shadow-emerald-600/30 transition-all flex items-center gap-2">
          ...
          <div className="relative">
            <div className="absolute inset-0 bg-gradient-to-tr from-emerald-500 to-emerald-300 rounded-[3rem] rotate-3 opacity-20 scale-105 blur-lg"></div>
            <img 
      ...
      <section id="services" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h3 className="text-emerald-600 font-bold uppercase tracking-wider mb-2">Dịch vụ của chúng tôi</h3>
            <h2 className="text-4xl font-black text-slate-900">Giải pháp hoàn hảo cho bạn</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {[ 
              { title: "Khám tổng quát", desc: "Dịch vụ chất lượng cao, thực hiện bởi đội ngũ chuyên nghiệp." },
              { title: "Nhi khoa", desc: "Giải pháp toàn diện đáp ứng mọi nhu cầu của bạn." },
              { title: "Xét nghiệm", desc: "Cam kết mang lại sự hài lòng tuyệt đối cho mọi khách hàng." }
            ].map((srv, i) => (
              <div key={i} className="bg-slate-50 rounded-3xl p-8 border border-slate-100 hover:shadow-xl transition-shadow group">
                ...
                <a href="#" className="text-emerald-600 font-bold flex items-center gap-1 group-hover:gap-2 transition-all">
