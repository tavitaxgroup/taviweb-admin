  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await BookingService.createResource(user.tenant_id, {
        name: formData.name,
        type: template === 'salon' ? 'staff' : 'table',
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-slate-200 flex flex-col h-full animate-in fade-in duration-500 relative">
      <div className="bg-slate-50 border-b border-slate-200 px-6 py-5 flex items-center justify-between rounded-t-3xl">
        <div>
          <h2 className="text-xl font-extrabold text-slate-800">Quản lý Tài nguyên ({getResourceTitle(template)})</h2>
          <p className="text-sm text-slate-500 font-medium">{template === 'salon' ? 'Danh sách nhân sự thực hiện dịch vụ' : 'Danh sách các bàn phục vụ tại nhà hàng'}</p>
        </div>
        <button onClick={() => setIsModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl font-bold text-sm shadow-md shadow-indigo-600/20 flex items-center gap-2 transition-colors">
          <Plus className="w-4 h-4" /> Thêm {getResourceLabel(template)} mới
        </button>
      </div>
      ...
      <div className="px-6 py-4 flex items-center justify-between border-b border-slate-100">
        <div className="relative">
          ...
          <input type="text" placeholder={`Tìm kiếm ${getResourceLabel(template)}...`} className="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 w-64" />
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <h3 className="font-bold text-slate-800 text-lg">Thêm {getResourceLabel(template)} mới</h3>
            ...
            <form onSubmit={handleAdd} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">Tên {getResourceLabel(template)} <span className="text-red-500">*</span></label>
                <input required autoFocus type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-sm" placeholder={`VD: ${template === 'salon' ? 'Nguyễn Văn A' : 'Bàn VIP 1'}`} />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">Vị trí / Sức chứa</label>
                <input type="text" value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})} className="w-full px-4 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-sm" placeholder={`VD: ${template === 'salon' ? 'Senior Barber' : '4 khách'}`} />
              </div>
