...
async function extractContactableLeadsFixed() {
  ...
  for (const ind of top4) {
    ...
    industryLeads.sort((a, b) => {
      ...
      const aScore = (aPhone ? 1 : 0) + (aFb ? 1 : 0);
      const bScore = (bPhone ? 1 : 0) + (bFb ? 1 : 0);
  ...
  let mdContent = '# Danh Sách 200 Khách Hàng Tiềm Năng (Có SĐT / Fanpage)\n\n';
