...
async function runQualification() {
  console.log(`[QUALIFICATION AGENT] Khởi động chế độ kiểm chứng liên tục (Continuous Mode)...`);
  ...
  while (true) {
    try {
      ...
      const { data: leads, error } = await supabase
        ...
        .is('website', null)
        .limit(100); // Lấy mỗi lần 100 khách để giải phóng RAM
      ...
      if (!leads || leads.length === 0) {
        console.log("Hiện tại đã hết khách mới cần kiểm chứng. Đang chờ 30 giây để kiểm tra lại...");
        await delay(30000);
        continue;
      }

      console.log(`\n======================================`);
      console.log(`Tìm thấy ${leads.length} Leads cần xác thực. Đang khởi động trình duyệt...`);
      ...
      for (let i = 0; i < leads.length; i++) {
        ...
        console.log(`\n[SEARCH] Đang kiểm chứng (${i+1}/${leads.length}): ${name}`);
      ...
      await browser.close();
      console.log(`\n[HOÀN TẤT] Quá trình kiểm chứng mẻ 100 khách đã xong! Sẽ lấy mẻ tiếp theo sau 5 giây...`);
      await delay(5000);
      
    } catch (globalError) {
      console.error("[LỖI GLOBAL] Tiến trình gặp lỗi lớn, sẽ thử lại sau 30 giây:", globalError);
      await delay(30000);
    }
  }
}
